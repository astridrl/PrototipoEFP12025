...
bool usuarios::modificarUsuario() {
    ...
    while (!file.eof()) {
        ...
        if (searchId == currentId) {
            found = true;
            //muestra los datos actuales para que el usuario sepa que está modificando
            cout << "\n\t\tDatos actuales:" << endl;
            ...
            //el mismo sistema para la contraseña
            char caracter;
            caracter = getch();
            newPass = "";
            while (caracter != 13) {
...
bool usuarios::eliminarUsuario() {
    ...
    while (!file.eof()) {
        ...
        if (searchId == currentId) {
            ...
            //se pide confirmar si esta seguro de la eliminación
            char confirmar;
